...
function patchAttr(md, tagname, attrname, classlist) {

    function defRender(tokens, idx, options, env, self) {
        return self.renderToken(tokens, idx, options);
    };
    // Save the original rendering rule for table_open (if any)
    const defaultRender = md.renderer.rules.table_open || defRender
    ...
    md.renderer.rules[`${tagname}_open`] = (tokens, idx, options, env, self) => {
        ...
        if (classIndex < 0) {
            token.attrPush([attrname, classlist]); // add new attribute
        } else {
            token.attrs[classIndex][1] += ` ${classlist}`; // append new class
        }
...
async function markdown(name) {
    ...
    document.getElementById('markdown').markdown = markdownText;
