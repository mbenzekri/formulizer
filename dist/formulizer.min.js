 ...
 * Copyright 2017 Google LLC
 * SPDX-License-Identifier: BSD-3-Clause
 */let bs=class extends ns{static get styles(){return[...super.styles,l`
                ul {
                    max-height: 300px; 
                    overflow-y: scroll
                }
            ...
            </div>`}toggleItem(t){if(null==this.value&&(this.value=[]),this.value.includes(t)){const e=this.value.indexOf(t);e>=0&&this.value.splice(e,1)}else this.value.push(t)}getItems(){const t=this.schema.items?.enum,e=this.parent;if(t)return t.reduce(((t,s)=>(this.evalExpr("filter",this.schema.items,s,e[this.key],-1)&&t.push({label:String(s),value:s}),t)),[]);const s=this.schema.items?.oneOf;return s?s.reduce(((t,e)=>(this.evalExpr("filter",e,e.const,this.parent[this.key],-1)&&t.push({label:e.title??e.description??e.const,value:e.const}),t)),[]):[]}};bs=s([i("fz-enum-array")],bs);let gs=class extends qt{modal;backdrop;validable=!1;#d="Dialogue";get modalTitle(){return this.#d}set modalTitle(t){this.#d=t}#p="Valider";get okLabel(){return this.#p}set okLabel(t){this.#p=t}#f="Annuler";get dismissLabel(){return this.#f}set dismissLabel(t){this.#f=t}static get styles(){return[...super.styles,l`
            .modal-body {
